...
"""22.01.06 COCO Instance Segmentation dataset"""
"""22.01.14 Support loading instances of specific categories"""
class COCOInsDataset(Dataset):
    """
    COCO dataset class.
    """
    ...
    def pull_item(self, index):
        ...
        # mask_vis = np.concatenate([np.zeros((mask_rsz.shape[0], mask_rsz.shape[1], 1)), mask_rsz], axis=-1) # (H, W, C+1)
        # mask_indice = np.argmax(mask_vis, axis=-1).astype(np.uint8) # (H, W)
        ...
        # img_E.save(os.path.join(prj_root, "mask.png"))
        # cv2.imwrite(os.path.join(prj_root, "img.jpg"), img)
        # import sys
        # sys.exit(0)
        # img: resized image, res: coordinates and class id on the resized image
        return img, res.copy(), img_info, np.array([id_]), mask_rsz
    ...
    def __getitem__(self, index):
        ...
        """
        img, target, img_info, img_id, mask_rsz = self.pull_item(index)
