...
from unicorn.core import launch
from unicorn.exp import get_exp
from unicorn.utils import configure_nccl, fuse_model, get_local_rank, get_model_info, setup_logger
...
def make_parser():
    ...
    # MOTS
    parser.add_argument("--mots", dest="mots", default=False, action="store_true", help="test mots.")
    parser.add_argument("--mask_thres", type=float, default=0.3, help="mask threshold for binarizing")
    return parser
...
def main(exp, args, num_gpu):
    ...
    # for MOTS, we don't use cudnn for better speed
    if not args.mots:
    ...
    if args.mots:
        evaluator.evaluate_omni_mots(
            model, is_distributed, args.fp16, trt_file, decoder, exp.test_size, results_folder, \
                grid_sample=exp.grid_sample, exp=exp)
        # we evaluate MOTS with another toolkit.
        logger.info("MOTS is completed.")
        import sys
        sys.exit(0)
    else:
        *_, summary = evaluator.evaluate_omni(
            model, is_distributed, args.fp16, trt_file, decoder, exp.test_size, results_folder, \
