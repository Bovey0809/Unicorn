...
class GOT10KDataset(BaseDataset):
    ...
    def _construct_sequence(self, sequence_name):
        anno_path = f'{self.base_path}/{sequence_name}/groundtruth.txt'

        ground_truth_rect = load_text(anno_path, delimiter=',', dtype=np.float64)

        frames_path = f'{self.base_path}/{sequence_name}'
    ...
    def _get_sequence_list(self, split):
        with open(f'{self.base_path}/list.txt') as f:
            sequence_list = f.read().splitlines()

        if split == 'ltrval':
            with open(f'{self.env_settings.dataspec_path}/got10k_val_split.txt') as f:
